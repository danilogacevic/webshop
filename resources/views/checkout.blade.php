    ...
    @foreach($products as $product)
        ...
        @if(session('product_'.$product->id))
            ...
            <tr>
                ...
                <td data-title="Qty">
                    <div class="input-group input-group-lg">
                                                <span class="input-group-btn">
                                                    <a href="{{route('decreaseAmount',$product->id)}}" type="button" class="btn-e btn-e-primary btn-e-number" disabled="disabled" data-type="minus" data-field="quant[2]">
                                                        <i class="fa fa-minus"></i>
                                                    </a>
                                                </span>
                        <input type="text" name="quant[2]" class="form-control input-number" data-min="1" data-max="10" value="{{session('product_'.$product->id)}}">
                        <span class="input-group-btn">
                                                    <a href="{{route('increaseAmount',$product->id)}}" type="button" class="btn-e btn-e-primary btn-e-number" data-type="plus" data-field="quant[2]">
                ...
                <td data-title="Remove"><a href="{{route('removeProduct',$product->id)}}" class="remove"><i class="fa fa-remove"></i></a></td>
            </tr>
...
@section('total_cart')
    ...
    <div class="col-md-12">
        <div class="cart-total pull-right">
            <p class="subtotal">
                <label>Sub total :</label> <span class="primary">${{session('item_total')}}</span></p>
            <h6>Discount code :</h6>
            ...
            <p class="grand-total">
                <label>Grand total :</label> <span class="primary">${{session('item_total')}}</span></p>
